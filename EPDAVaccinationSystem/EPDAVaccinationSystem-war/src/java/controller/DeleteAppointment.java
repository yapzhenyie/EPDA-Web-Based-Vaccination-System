...
public class DeleteAppointment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String redirectUrl = request.getParameter("deleteAppointmentRedirectUrl");
        try {
            appointmentId = Long.parseLong(request.getParameter("deleteAppointmentId"));
            ...
            if (appointment != null) {
                try {
                    ...
                    if (clinicStaff != null && vaccinator != null) {
                        clinicStaff.getAppointments().remove(appointment);
                        clinicStaffFacade.edit(clinicStaff);

                        vaccinator.getAppointments().remove(appointment);
                        ...
                        session.setAttribute(ConstantSession.SuccessMessage, ConstantMessage.SuccessfulDeleteAppointment);
                        response.sendRedirect(redirectUrl);
                    } else {
                        session.setAttribute(ConstantSession.Validate, ConstantMessage.Error);
                        session.setAttribute(ConstantSession.ValidateMessage, ConstantMessage.UnexpectedErrorOccurred);
